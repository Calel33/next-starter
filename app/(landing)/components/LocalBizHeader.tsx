"use client";

import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuSeparator,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";

import { Search, Building, Plus, List, ChevronDown } from "lucide-react";
import { useState, useMemo } from "react";
import { useUser, SignInButton, UserButton } from "@clerk/nextjs";
import { useTheme } from "next-themes";
import { dark } from '@clerk/themes';
import { useRouter } from "next/navigation";
import { useUserRole } from "@/lib/auth-utils";
import { HeaderRoleSelection } from "@/components/custom/RoleSelectionModal";

export function LocalBizHeader() {
  const [searchQuery, setSearchQuery] = useState("");
  const { user, isSignedIn } = useUser();
  const router = useRouter();
  const { theme } = useTheme();
  const { role, isOwner, isAdmin } = useUserRole();

  const appearance = useMemo(() => ({
    baseTheme: theme === "dark" ? dark : undefined,
  }), [theme]);

  // Configure UserButton with proper redirect for business users
  const userButtonProps = useMemo(() => ({
    appearance,
    afterSignOutUrl: "/",
    // Redirect business users to their dashboard after sign out
    ...(isOwner && { afterSignOutUrl: "/" }),
    ...(isAdmin && { afterSignOutUrl: "/" }),
  }), [appearance, isOwner, isAdmin]);

  const handleListBusinessClick = () => {
    // User is signed in, redirect to listing form
    router.push("/dashboard/owner/create");
  };

  return (
    <header className="flex items-center justify-between whitespace-nowrap border-b border-border px-10 py-3 bg-background">
      <div className="flex items-center gap-8">
        {/* Logo Section */}
        <div className="flex items-center gap-4 text-foreground">
          <div className="size-4">
            <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M39.475 21.6262C40.358 21.4363 40.6863 21.5589 40.7581 21.5934C40.7876 21.655 40.8547 21.857 40.8082 22.3336C40.7408 23.0255 40.4502 24.0046 39.8572 25.2301C38.6799 27.6631 36.5085 30.6631 33.5858 33.5858C30.6631 36.5085 27.6632 38.6799 25.2301 39.8572C24.0046 40.4502 23.0255 40.7407 22.3336 40.8082C21.8571 40.8547 21.6551 40.7875 21.5934 40.7581C21.5589 40.6863 21.4363 40.358 21.6262 39.475C21.8562 38.4054 22.4689 36.9657 23.5038 35.2817C24.7575 33.2417 26.5497 30.9744 28.7621 28.762C30.9744 26.5497 33.2417 24.7574 35.2817 23.5037C36.9657 22.4689 38.4054 21.8562 39.475 21.6262ZM4.41189 29.2403L18.7597 43.5881C19.8813 44.7097 21.4027 44.9179 22.7217 44.7893C24.0585 44.659 25.5148 44.1631 26.9723 43.4579C29.9052 42.0387 33.2618 39.5667 36.4142 36.4142C39.5667 33.2618 42.0387 29.9052 43.4579 26.9723C44.1631 25.5148 44.659 24.0585 44.7893 22.7217C44.9179 21.4027 44.7097 19.8813 43.5881 18.7597L29.2403 4.41187C27.8527 3.02428 25.8765 3.02573 24.2861 3.36776C22.6081 3.72863 20.7334 4.58419 18.8396 5.74801C16.4978 7.18716 13.9881 9.18353 11.5858 11.5858C9.18354 13.988 7.18717 16.4978 5.74802 18.8396C4.58421 20.7334 3.72865 22.6081 3.36778 24.2861C3.02574 25.8765 3.02429 27.8527 4.41189 29.2403Z"
                fill="currentColor"
              />
            </svg>
          </div>
          <h2 className="text-foreground text-lg font-bold leading-tight tracking-tight">
            LocalBiz
          </h2>
        </div>
        
        {/* Navigation */}
        <nav className="flex items-center gap-9">
          <a 
            className="text-foreground text-sm font-medium leading-normal hover:text-primary transition-colors" 
            href="#"
          >
            Home
          </a>
          <a 
            className="text-foreground text-sm font-medium leading-normal hover:text-primary transition-colors" 
            href="#"
          >
            Categories
          </a>
          <a 
            className="text-foreground text-sm font-medium leading-normal hover:text-primary transition-colors" 
            href="#"
          >
            Deals
          </a>
          <a 
            className="text-foreground text-sm font-medium leading-normal hover:text-primary transition-colors" 
            href="#"
          >
            About
          </a>
        </nav>
      </div>
      
      <div className="flex flex-1 justify-end gap-8">
        {/* Search Input */}
        <div className="relative min-w-40 max-w-64">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input
            placeholder="Search"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="pl-10 bg-muted border-none h-10"
          />
        </div>
        
        {/* List Business Button */}
        {isSignedIn ? (
          <Button
            variant="secondary"
            className="min-w-[84px] max-w-[480px] h-10 px-4 font-bold tracking-wide"
            onClick={handleListBusinessClick}
          >
            List Your Business
          </Button>
        ) : (
          <HeaderRoleSelection>
            <Button
              variant="secondary"
              className="min-w-[84px] max-w-[480px] h-10 px-4 font-bold tracking-wide"
            >
              List Your Business
            </Button>
          </HeaderRoleSelection>
        )}
        
        {/* Business Dashboard - Desktop (Owner Only) */}
        {isSignedIn && isOwner && (
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push("/dashboard/owner")}
            className="hidden sm:flex"
          >
            Dashboard
          </Button>
        )}

        {/* Admin Dashboard - Desktop (Admin Only) */}
        {isSignedIn && isAdmin && (
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push("/dashboard/admin")}
            className="hidden sm:flex"
          >
            Admin Panel
          </Button>
        )}

        {/* Business Dashboard - Mobile Dropdown (Owner Only) */}
        {isSignedIn && isOwner && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="sm:hidden">
                <Building className="h-4 w-4 mr-1" />
                <ChevronDown className="h-3 w-3" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem onClick={() => router.push("/dashboard/owner")}>
                <Building className="h-4 w-4 mr-2" />
                My Dashboard
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => router.push("/dashboard/owner/listings")}>
                <List className="h-4 w-4 mr-2" />
                My Listings
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => router.push("/dashboard/owner/create")}>
                <Plus className="h-4 w-4 mr-2" />
                Add Listing
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        )}

        {/* Admin Dashboard - Mobile (Admin Only) */}
        {isSignedIn && isAdmin && (
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push("/dashboard/admin")}
            className="sm:hidden"
          >
            Admin
          </Button>
        )}

        {/* User Profile - Only show when authenticated */}
        {isSignedIn && (
          <UserButton {...userButtonProps} />
        )}
      </div>
    </header>
  );
}
